...
router.patch("/tasks/:id", async (req, res) => {
  try {
    ...
    // When unblocking: append human_input to description so re-dispatched agent sees it
    if (updates.human_input && updates.status === "todo") {
      // Fetch current task to get existing description
      const { data: current } = await supabase.from("agent_tasks").select("description").eq("id", req.params.id).single();
      if (current && current.description) {
        updates.description = current.description + "\n\n---\n**Human Input (unblock):** " + updates.human_input;
      } else {
        updates.description = "**Human Input (unblock):** " + updates.human_input;
      }
    }
