...
export default function TaskDetailModal({ task, onClose, onStatusChange, isMobile, isTablet, progress, monitor }) {
  ...
  const handleDeploy = async () => {
    ...
    const timeout = setTimeout(() => controller.abort(), 150_000); // 2.5min timeout (backend polls ArgoCD for up to 2min)
    try {
      ...
      const msg = e.name === 'AbortError' ? 'Deploy timed out — check ArgoCD status manually' : (e.message || "Deploy failed");
      setDeployError(msg);
